...
% Output:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%(R1_DTM,deltaX,deltaY,[xMinF,xMaxF],[yMinF,yMaxF]);
% rectangleIndices_for_integration(deltaX,deltaY,R_minF_DTM,R_maxF_DTM);
%function [nX, nY]=rectangleIndices_for_integration(R1,deltaX,deltaY,x,y)

function [nX, nY]=rectangleIndices_for_integration(deltaX,deltaY,R_minF_DTM,R_maxF_DTM)
%X1=R1(1);  Y1=R1(2); 

nX(1)=ceil((R_minF_DTM(1))/deltaX+1/2);
nX(2)=floor((R_maxF_DTM(1))/deltaX+1/2);
nY(1)=ceil((R_minF_DTM(2))/deltaY+1/2);
nY(2)=floor((R_maxF_DTM(2))/deltaY+1/2);
